...
| ----------------------------------------------- | -------------------------: | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| Broker as single enforcement boundary           |                Implemented | `atb-gateway-go/cmd/broker/main.go`, Helm chart `charts/atb`                                         | ✅ Connectors implemented with egress allowlists, per-connector rate limiting, configurable headers. See `config/connectors.example.json`. |
| SPIFFE/SPIRE workload identity (secret-less)    |                Implemented | `atb-gateway-go/cmd/broker/main.go` (Workload API), `spire/`, `docs/k8s-quickstart.md`, `charts/atb` | ✅ X509-SVID for mTLS, JWT-SVID for external APIs, Federation for cross-domain trust, Platform↔SPIFFE binding. |
...
| Central policy enforcement (OPA)                |                Implemented | `opa/policy/poa.rego`, broker `OPA_DECISION_URL`                                                     | ✅ 145+ enterprise actions across risk tiers with constraint validation. See `docs/enterprise-actions.md`.                       |
| Semantic/prompt-injection firewall              |                Implemented | broker `semanticGuardrails(...)`, `GUARDRAILS_URL`                                                   | Local pattern matching + optional external service (e.g., Azure AI Content Safety). Metrics: `atb_guardrails_requests_total`.    |
| Centralised audit trail schema                  |                Implemented | `schemas/audit-event.schema.json`, `docs/audit.md`, `ImmutableStorageSink`                           | ✅ Immutable storage with Azure Blob/S3 Object Lock, hash-chain tamper evidence, 7-year retention.                               |
| "Non-transferable" credentials                  |                Implemented | PoA `sub` bound to SPIFFE ID, `POA_SINGLE_USE` replay protection                                     | ✅ jti-based replay cache with TTL. Sub must match caller's SPIFFE ID. External PoA binding via platform OIDC `X-Platform-Token`. |
| Operating model / RACI                          |                Implemented | `docs/operating-model.md`                                                                            | ✅ RACI matrix, approval flows, risk thresholds, mandate templates, escalation paths.                                            |
