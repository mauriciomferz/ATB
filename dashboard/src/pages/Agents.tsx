  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-900">Registered Agents</h1>
      ...
      {/* Agent Legend */}
      <div className="card">
        <h3 className="text-lg font-medium mb-4">Risk Profiles</h3>
        <div className="grid grid-cols-3 gap-4">
          <div className="flex items-center">
            <span className="w-3 h-3 rounded-full bg-success-500 mr-2"></span>
            <span className="text-sm text-gray-600">LOW - Read-only, non-sensitive</span>
          </div>
          <div className="flex items-center">
            <span className="w-3 h-3 rounded-full bg-warning-500 mr-2"></span>
            <span className="text-sm text-gray-600">MEDIUM - Writes, bulk operations</span>
          </div>
          <div className="flex items-center">
            <span className="w-3 h-3 rounded-full bg-danger-500 mr-2"></span>
            <span className="text-sm text-gray-600">HIGH - Financial, admin actions</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="card">
      <div className="flex items-start justify-between">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">{trustDomain}</p>
          </div>
        </div>
      ...
      <dl className="mt-4 space-y-2">
        ...
        <div className="flex justify-between text-sm">
          ...
          <dd className="text-gray-900 font-medium">{agent.requestCount.toLocaleString()}</dd>
        </div>
        <div className="flex justify-between text-sm">
          <dt className="text-gray-500">Allow rate</dt>
          <dd className={`font-medium ${agent.allowRate >= 0.95 ? 'text-success-600' : agent.allowRate >= 0.9 ? 'text-warning-600' : 'text-danger-600'}`}>
      ...
      <div className="mt-4 pt-4 border-t">
        <code className="text-xs text-gray-500 break-all">{agent.spiffeId}</code>
      </div>
    </div>
  );
}
