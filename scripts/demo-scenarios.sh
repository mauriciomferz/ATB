...
show_menu() {
    ...
    echo ""
    echo "  ${GREEN}1${NC}) Low Risk - Health Check (auto-approved)"
    echo "  ${GREEN}2${NC}) Medium Risk - CRM Read (1 approver)"
    echo "  ${GREEN}3${NC}) High Risk - SAP Payment (dual control)"
    echo "  ${YELLOW}4${NC}) Denied - Missing Approval"
    echo "  ${YELLOW}5${NC}) Denied - Expired Token"
    echo "  ${GREEN}6${NC}) Multi-Leg Transaction"
    echo "  ${YELLOW}7${NC}) Rate Limited"
    echo "  ${CYAN}8${NC}) Audit Trail Example"
    echo "  ${CYAN}9${NC}) GDPR Compliant Access"
    echo "  ${RED}10${NC}) Emergency Break Glass"
    echo ""
    echo "  ${BOLD}a${NC}) Run all scenarios"
    echo "  ${BOLD}q${NC}) Quit"
    echo ""
}
